...
def main(ifile, gfile, ofile):
    ...
    result_dict = {}
    for fil in indata:
        sample = fil.split('/')[-1].split('.')[0].replace('query_', '')
        with open(fil,'r') as f:
            fdata = [line.strip().split('\t')[0]+'-'+line.strip().split('\t')[1]+'-'+line.strip().split('\t')[2]+'-'+sample+'-'+line.strip().split('\t')[3] for line in f.readlines()]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="Example: get_SVs_for_gene.py sv_list SVs_for_gene.txt")
    parser.add_argument('ifile', help='input file: list for query*.dat')
