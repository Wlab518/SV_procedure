...
def main(infile, sfile):
    ...
    for strain in strain_list:
        ...
        with open('{0}/result_stat/barplot/{1}/run_barplot.r'.format(path, strain),'w') as o1:
            ...
            o1.write('ggplot(stat, aes(sv_type, weight = numbers, fill = Samples)) +\n')
            o1.write('  geom_bar(color = "black", width = .9, position = \'dodge\') +\n')
            ...
            o1.write('  scale_fill_manual(values=c(')
            for sap, col in zip(color_dict.values(), ["white","gray"]):
                for s in sap:
                    if s != sap[-1]:
                        o1.write('"{0}"="{1}", '.format(s, col))
                    else:
                        if col == "gray":
                            o1.write('"{0}"="{1}" '.format(s, col))
                        else:
